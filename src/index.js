module.exports = function toReadable(number) {
    ...
    if (number <= 10) return frst[number];

    if (number > 10 && number <= 20) return scnd[number.toString()[1]];

    if (number > 20 && number < 100) {
        if (number.toString()[1] === '0') return thrd[number.toString()[0]];
        else return thrd[number.toString()[0]] + ' ' + frst[number.toString()[1]];
    }
